use std::{collections::HashMap, env, time::Duration};

use reqwest::{
    blocking::Client,
    header::{HeaderMap, HeaderValue},
    StatusCode,
};
use serde::{Deserialize, Serialize};

// The structs follow the json schema defined in the GitHub docs.
// This causes there to be unused fields which then throw a dead
// code warning, these warnings may be suppressed.

#[derive(Debug, Deserialize)]
#[serde(untagged)]
pub enum GetContentResponse {
    Struct(GetContentFileResponse),
    List(Vec<GetContentDirectoryResponse>),
}

#[derive(Debug, Deserialize)]
pub struct GetContentFileResponse {
    pub path: String,
    pub content: String,
}

#[derive(Debug, Deserialize)]
pub struct GetContentDirectoryResponse {
    #[serde(rename = "type")]
    pub _type: String,
    pub path: String,
}

#[derive(Debug, Clone, Deserialize)]
pub struct GetBranchResponse {
    pub commit: Commit,
}

#[derive(Debug, Clone, Deserialize)]
pub struct Commit {
    pub sha: String,
    pub commit: CommitCommit,
}

#[derive(Debug, Clone, Deserialize)]
pub struct CommitCommit {
    pub tree: Tree,
}

#[derive(Debug, Clone, Deserialize)]
pub struct Tree {
    pub sha: String,
}

#[derive(Debug, Clone, Serialize)]
struct CreateBlobRequestBody {
    content: String,
    encoding: String,
}

#[derive(Debug, Clone, Deserialize)]
pub struct CreateBlobResponse {
    pub sha: String,
}

#[derive(Debug, Serialize)]
struct CreateTreeRequestBody {
    pub base_tree: String,
    pub tree: Vec<RequestFile>,
}

#[derive(Debug, Serialize)]
struct RequestFile {
    pub path: String,
    pub mode: String,
    #[serde(rename = "type")]
    pub _type: String,
    pub sha: String,
}

#[derive(Debug, Deserialize)]
pub struct CreateTreeResponse {
    pub sha: String,
}

#[derive(Debug, Deserialize)]
pub struct ResponseFile {
    #[serde(rename = "type")]
    pub _type: String,
}

#[derive(Debug, Serialize)]
struct CreateCommitRequestBody {
    message: String,
    tree: String,
    parents: Vec<String>,
    author: Author,
}

#[derive(Debug, Serialize)]
struct Author {
    name: String,
    email: String,
}

#[derive(Debug, Deserialize)]
pub struct CreateCommitResponse {
    pub sha: String,
}

#[derive(Debug, Serialize)]
struct UpdateReferenceRequestBody {
    sha: String,
}

pub struct Requests {
    client: Client,
}

impl Requests {
    pub fn new() -> Result<Requests, ()> {
        let mut headers = HeaderMap::new();
        headers.insert(
            "Accept",
            HeaderValue::from_str("application/vnd.github+json").unwrap(),
        );
        headers.insert(
            "X-GitHub-Api-Version",
            HeaderValue::from_str("2022-11-28").unwrap(),
        );
        headers.insert(
            "Authorization",
            HeaderValue::from_str(format!("Bearer {}", env::var("PAT").unwrap()).as_str()).unwrap(),
        );

        match Client::builder()
            .user_agent("paperback-community/registry-manager")
            .default_headers(headers)
            .timeout(Duration::new(10, 0))
            .build()
        {
            Ok(client) => {
                println!("Created a request client");
                Ok(Requests { client })
            }
            Err(err) => {
                println!(
                    "Something went wrong while creating the request client: {}",
                    &err
                );
                Err(())
            }
        }
    }

    pub fn get_files(
        &self,
        repository: &String,
        path: &String,
        branch: &String,
    ) -> Result<GetContentResponse, bool> {
        let p_response = self
            .client
            .get(format!(
                "https://api.github.com/repos/{}/contents/{}?ref={}",
                &repository, &path, &branch
            ))
            .send();

        match p_response {
            Ok(raw_response) => {
                match raw_response.status() {
                    StatusCode::OK => {}
                    StatusCode::NOT_FOUND => {
                        eprintln!("The requested files were not found");
                        return Err(true);
                    }
                    _ => {
                        eprintln!(
                            "The response was undesired, status code: {}",
                            &raw_response.status(),
                        );
                        return Err(false);
                    }
                }

                match raw_response.json::<GetContentResponse>() {
                    Ok(response) => Ok(response),
                    Err(err) => {
                        eprintln!(
                            "Something went wrong while deserializing the response to JSON: {}",
                            &err
                        );
                        Err(false)
                    }
                }
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(false)
            }
        }
    }

    pub fn get_branch(
        &self,
        repository: &String,
        branch: &String,
    ) -> Result<GetBranchResponse, ()> {
        let p_response = self
            .client
            .get(format!(
                "https://api.github.com/repos/{}/branches/{}",
                &repository, &branch
            ))
            .send();

        match p_response {
            Ok(raw_response) => {
                if raw_response.status() != 200 {
                    eprintln!(
                        "The response was undesired, status code: {}",
                        &raw_response.status(),
                    );
                    return Err(());
                }

                match raw_response.json::<GetBranchResponse>() {
                    Ok(response) => Ok(response),
                    Err(err) => {
                        eprintln!(
                            "Something went wrong while deserializing the response to JSON: {}",
                            &err
                        );
                        Err(())
                    }
                }
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(())
            }
        }
    }

    pub fn create_blob(&self, content: String, encoding: String) -> Result<CreateBlobResponse, ()> {
        let body = CreateBlobRequestBody { content, encoding };

        let p_body_string = serde_json::to_string(&body);

        let p_response;
        match p_body_string {
            Ok(body_string) => p_response = self
                .client
                .post("https://api.github.com/repos/paperback-community/extensions-test/git/blobs")
                .body(body_string)
                .send(),
            Err(err) => {
                eprintln!(
                    "Something went wrong while serializing the request body to JSON: {}",
                    &err
                );
                return Err(());
            }
        }

        match p_response {
            Ok(raw_response) => {
                if raw_response.status() != 201 {
                    eprintln!(
                        "The response was undesired, status code: {}",
                        &raw_response.status(),
                    );
                    return Err(());
                }

                let raw_value_response = raw_response.json::<serde_json::Value>().unwrap();

                match serde_json::from_value::<CreateBlobResponse>(raw_value_response) {
                    Ok(response) => Ok(response),
                    Err(err) => {
                        eprintln!(
                            "Something went wrong while deserializing the response to JSON: {}",
                            &err
                        );
                        Err(())
                    }
                }
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(())
            }
        }
    }

    pub fn create_tree(
        &self,
        base_tree: String,
        updated_extensions: Vec<(String, HashMap<String, String>)>,
    ) -> Result<CreateTreeResponse, ()> {
        let mut tree = vec![];
        for updated_extension in updated_extensions {
            for updated_extension_file in updated_extension.1.keys() {
                let file = RequestFile {
                    path: updated_extension_file.to_string(),
                    mode: String::from("100644"),
                    _type: String::from("blob"),
                    sha: updated_extension
                        .1
                        .get(&updated_extension_file.to_string())
                        .unwrap()
                        .to_string(),
                };

                tree.push(file);
            }
        }

        let body = CreateTreeRequestBody { base_tree, tree };

        let p_body_string = serde_json::to_string(&body);

        let p_response;
        match p_body_string {
            Ok(body_string) => p_response = self
                .client
                .post("https://api.github.com/repos/paperback-community/extensions-test/git/trees")
                .body(body_string)
                .send(),
            Err(err) => {
                eprintln!(
                    "Something went wrong while serializing the request body to JSON: {}",
                    &err
                );
                return Err(());
            }
        }

        match p_response {
            Ok(raw_response) => {
                if raw_response.status() != 201 {
                    eprintln!(
                        "The response was undesired, status code: {}",
                        &raw_response.status(),
                    );
                    return Err(());
                }

                let raw_value_response = raw_response.json::<serde_json::Value>().unwrap();

                match serde_json::from_value::<CreateTreeResponse>(raw_value_response) {
                    Ok(response) => Ok(response),
                    Err(err) => {
                        eprintln!(
                            "Something went wrong while deserializing the response to JSON: {}",
                            &err
                        );
                        Err(())
                    }
                }
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(())
            }
        }
    }

    pub fn create_commit(
        &self,
        message: String,
        tree_sha: String,
        parent_commit_sha: String,
        author_name: String,
        author_email: String,
    ) -> Result<CreateCommitResponse, ()> {
        let body = CreateCommitRequestBody {
            message,
            tree: tree_sha,
            parents: vec![parent_commit_sha],
            author: Author {
                name: author_name,
                email: author_email,
            },
        };

        let p_body_string = serde_json::to_string(&body);

        let p_response;
        match p_body_string {
            Ok(body_string) => p_response = self
                .client
                .post(
                    "https://api.github.com/repos/paperback-community/extensions-test/git/commits",
                )
                .body(body_string)
                .send(),
            Err(err) => {
                eprintln!(
                    "Something went wrong while serializing the request body to JSON: {}",
                    &err
                );
                return Err(());
            }
        }

        match p_response {
            Ok(raw_response) => {
                if raw_response.status() != 201 {
                    eprintln!(
                        "The response was undesired, status code: {}",
                        &raw_response.status(),
                    );
                    return Err(());
                }

                let raw_value_response = raw_response.json::<serde_json::Value>().unwrap();

                match serde_json::from_value::<CreateCommitResponse>(raw_value_response) {
                    Ok(response) => Ok(response),
                    Err(err) => {
                        eprintln!(
                            "Something went wrong while deserializing the response to JSON: {}",
                            &err
                        );
                        Err(())
                    }
                }
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(())
            }
        }
    }

    pub fn update_reference(&self, commit_sha: String) -> Result<(), ()> {
        let body = UpdateReferenceRequestBody { sha: commit_sha };

        let p_body_string = serde_json::to_string(&body);

        let p_response;
        match p_body_string {
            Ok(body_string) => p_response = self
                .client
                .post(
                    "https://api.github.com/repos/paperback-community/extensions-test/git/refs/heads/master",
                )
                .body(body_string)
                .send(),
            Err(err) => {
                eprintln!(
                    "Something went wrong while serializing the request body to JSON: {}",
                    &err
                );
                return Err(());
            }
        }

        match p_response {
            Ok(raw_response) => {
                if raw_response.status() != 200 {
                    eprintln!(
                        "The response was undesired, status code: {}",
                        &raw_response.status(),
                    );
                    return Err(());
                }
                Ok(())
            }
            Err(err) => {
                eprintln!("Something went wrong while making the request: {}", &err);
                Err(())
            }
        }
    }
}
